...
from api.serializers import QuotaSerializer, ResourceSerializer, UserSerializer
...
@method_decorator(
    ...
    decorator=swagger_auto_schema(
        ...
        responses={400: "User already exist"},
    ),
)
...
class RetrieveDeleteUserViewSet(
...
@method_decorator(
    ...
    decorator=swagger_auto_schema(
        operation_description="API endpoint that provides registration of the user",
        responses={
            400: "User is prohibited from creating resources by Admin\n"
...
class UserResourceViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        # add id from token to data for resource creation
        serializer = self.get_serializer(
            instance, data={**request.data, "user_id": request.user.id}, partial=partial
        )
        serializer.is_valid(raise_exception=True)
