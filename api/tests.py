...
class AdminTests(TestCase):
    ...
    def setUp(self) -> None:
        self.user = QuotaUser.objects.create_user(
            username="test", email="test@test.com", password="password"
        )
        Quota.objects.create(id=self.user)

        self.admin = QuotaUser.objects.create_superuser(
    ...
    def test_admin_set_quota(self):
        ...
        request = client.put(
            f"/admin/quotas/{self.user.id}/",
            {"id": self.user.id, "quota": f"{pow(2,63) -1}", "allowed": "true"},
            format="json",
        )
        ...
        self.assertTrue(
            request.data
            == {"id": self.user.id, "quota": pow(2, 63) - 1, "allowed": True}
        )
        ...
        request = client.patch(
            f"/admin/quotas/{self.user.id}/",
            {"quota": "1", "allowed": "false"},
            format="json",
        )
        quota = Quota.objects.filter(id=self.user).last()
        self.assertTrue(request.status_code == 200)
        self.assertTrue(
            request.data == {"id": self.user.id, "quota": 1, "allowed": False}
        )
        ...
        request = client.patch(
            f"/admin/quotas/{self.user.id}/",
            {"quota": f"{pow(2,63)}", "allowed": "false"},
            format="json",
        )
        quota = Quota.objects.filter(id=self.user).last()
        self.assertTrue(request.status_code == 400)
    ...
    def test_admin_resources_basic(self):
        ...
        request = client.post(
            "/admin/resources/",
            {"resource": "3", "user_id": self.user.id},
            format="json",
        )
        ...
        request = client.put(
            f"/admin/resources/{user_res.id}/",
            {"id": user_res.id, "resource": "Putted resource", "user_id": self.user.id},
            format="json",
        )
        self.assertTrue(request.status_code == 200)
        self.assertTrue(
            Resource.objects.filter(id=user_res.id).last().resource == "Putted resource"
        )

        request = client.patch(
            f"/admin/resources/{user_res.id}/", {"resource": "Some res"}, format="json"
        )
        self.assertTrue(request.status_code == 200)
        self.assertTrue(
            Resource.objects.filter(id=user_res.id).last().resource == "Some res"
        )
...
class UserTests(TestCase):
    ...
    def setUp(self) -> None:
        ...
        Quota.objects.create(id=self.user)
        self.quoted_user = QuotaUser.objects.create_user(
            username="quoted_user", email="quoted_user@test.com", password="password"
        )
        Quota.objects.create(id=self.quoted_user, quota=1)
        self.restricted_user = QuotaUser.objects.create_user(
            username="restricted_user",
            email="restricted_user@test.com",
            password="password",
        )
    ...
    def test_user_login(self):
        ...
        for credentials in users_creds:
            with self.subTest(f"Test auth for {credentials}"):
                client = APIClient()
                request = client.post("/login/", credentials, format="json")
                self.assertTrue(request.status_code == 200)
    ...
    def test_resource_basics(self):
        ...
        for user in users:
            with self.subTest(f"Test resources usage for user {user}"):
                ...
                request = client.post(
                    "/resources/", {"resource": "test_1_resource"}, format="json"
                )
                self.assertTrue(request.status_code == 201)
    ...
    def test_resource_quoted(self):
        ...
        request = client.post(
            "/resources/", {"resource": "test_2_resource"}, format="json"
        )
        self.assertTrue(request.status_code == 400)
        ...
        request = client.put(
            "/resources/1/", {"resource": "test_2_resource"}, format="json"
        )
        self.assertTrue(request.status_code == 200)
        self.assertTrue(Resource.objects.filter(resource="test_2_resource"))

        request = client.patch(
            "/resources/1/", {"resource": "test_3_resource"}, format="json"
        )
        self.assertTrue(request.status_code == 200)
        self.assertTrue(Resource.objects.filter(resource="test_3_resource"))
    ...
    def test_resource_restricted(self):
        ...
        Resource.objects.create(
            resource="test_1_resource", user_id=self.restricted_user
        )
